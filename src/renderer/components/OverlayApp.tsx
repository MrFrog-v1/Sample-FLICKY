...
export function OverlayApp() {
  ...
  useEffect(() => {
    ...
    const unsubs = [
      ...
      window.flicky.onToggleTextInput(() => {
        setIsTextInputActive(true);
        // Focus needs a tiny delay to ensure the element is rendered and interactive
        setTimeout(() => inputRef.current?.focus(), 50);
      }),
      window.flicky.onLocateError((msg) => {
        setLocateError(msg);
    ...
    // Subscribe to AI response events for text bubble
    ...
    const onComplete = (text: string) => {
      ...
      setShowResponseBubble(true);
      // Auto-hide the response bubble after 8 seconds
    ...
    const unsubChunk = window.flicky.onAiResponseChunk(onChunk);
    const unsubComplete = window.flicky.onAiResponseComplete(onComplete);
  ...
  useEffect(() => {
    ...
    if (voiceState === 'processing') {
      // Clear old response when a new query starts processing
      setResponseText('');
      setShowResponseBubble(false);
      if (responseTimeoutRef.current) {
        clearTimeout(responseTimeoutRef.current);
  ...
  return (
    <div 
      ...
      style={{ pointerEvents: isTextInputActive ? 'auto' : 'none' }}
      onClick={() => {
        if (isTextInputActive) {
          window.flicky.cancelTextInput();
        ...
        <>
          ...
          {isTextInputActive && (
            <input
              ...
              placeholder="Ask Flicky..."
              onChange={(e) => setTextInputValue(e.target.value)}
              onKeyDown={(e) => {
                if (e.key === 'Enter' && textInputValue.trim()) {
                  window.flicky.submitTextInput(textInputValue);
                  setIsTextInputActive(false);
                  setTextInputValue('');
          ...
          {showResponseBubble && responseText && (
            <div
              className="response-bubble"
              style={{
                left: companionPos.x + 32,
                top: companionPos.y + 28,
              }}
            >
              {responseText.replace(/\[LOCATE:[^\]]+\]/g, '').slice(0, 300)}
            </div>
          )}
