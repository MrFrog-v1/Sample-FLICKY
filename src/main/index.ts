...
try {
  ...
  console.error('[Flicky] Failed to load .env file:', err);
}
...
app.whenReady().then(() => {
  ...
  companion = new CompanionManager({
    ...
    onToggleTextInput: () => {
      ...
      sendToOverlays(IPC.TOGGLE_TEXT_INPUT);
    },
  });
  ...
  // Register text input hotkey
  globalShortcut.register('CommandOrControl+Alt+Z', () => {
    companion.toggleTextInput();
  });
  ...
  // Text Input handlers
  ipcMain.on(IPC.SUBMIT_TEXT_INPUT, (_e, text: string) => {
    setOverlayInteractable(false);
    void companion.handleTextQuery(text);
  });
  ipcMain.on(IPC.CANCEL_TEXT_INPUT, () => {
    setOverlayInteractable(false);
  });
...
function setOverlayInteractable(enabled: boolean): void {
  for (const win of overlayWindows) {
    if (!win.isDestroyed()) {
      if (enabled) {
