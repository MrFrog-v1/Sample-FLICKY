...
export class GroqWhisperProvider implements TranscriptionProvider {
  ...
  async stop(): Promise<TranscriptionResult> {
    ...
    console.log(`[Flicky STT] Stopping recording. Captured ${chunkCount} chunks, total ${pcmData.length} bytes.`);

    // 16000 Hz * 2 bytes per sample * 0.1s = 3200 bytes minimum
    if (pcmData.length < 3200) {
      console.log('[Flicky STT] Audio too short, skipping transcription.');
      return { text: '', isFinal: true };
    }

    const wavBuffer = buildWav(pcmData, 16000, 1, 16);
    const model = settingsStore.get('groqTranscriptionModel');
    console.log(`[Flicky STT] Requesting transcription using model: ${model}`);
    ...
    if (!res.ok) {
      ...
      console.error(`[Flicky STT] Groq transcription error ${res.status}: ${errText}`);
      throw new Error(`Groq transcription error ${res.status}: ${errText}`);
    }
    
    const result = (await res.json()) as { text: string };
    console.log(`[Flicky STT] Transcribed text: "${result.text}"`);
