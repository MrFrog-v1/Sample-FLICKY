...
const api = {
  ...
  onToggleTextInput: (cb: () => void) => {
    const handler = () => cb();
    ipcRenderer.on(IPC.TOGGLE_TEXT_INPUT, handler);
    return () => ipcRenderer.removeListener(IPC.TOGGLE_TEXT_INPUT, handler);
  },

  onLocateError: (cb: (message: string) => void) => {
    const handler = (_e: Electron.IpcRendererEvent, msg: string) => cb(msg);
    ipcRenderer.on(IPC.LOCATE_ERROR, handler);
    return () => ipcRenderer.removeListener(IPC.LOCATE_ERROR, handler);
  },

  submitTextInput: (text: string): void => ipcRenderer.send(IPC.SUBMIT_TEXT_INPUT, text),
  cancelTextInput: (): void => ipcRenderer.send(IPC.CANCEL_TEXT_INPUT),
